...
export function WebOperationsDashboard() {
  ...
  const reminderTickets = tickets.filter((ticket) => ticket.status === "New" || ticket.status === "In progress" || ticket.status === "Dead");

  useEffect(() => {
    const reminderTimer = window.setInterval(() => {
      if (!reminderTickets.length) return;
      setRemindersAcknowledged(false);
      const message = `${reminderTickets.length} active guest request${reminderTickets.length === 1 ? " needs" : "s need"} an hourly follow-up.`;
      showToast(message);
      if ("Notification" in window && Notification.permission === "granted") new Notification("Intellistay operations reminder", { body: message });
    }, 60 * 60 * 1000);
    return () => window.clearInterval(reminderTimer);
  }, [reminderTickets.length]);
  ...
  const enableBrowserAlerts = async () => {
    ...
    showToast(permission === "granted" ? "Hourly browser reminders are enabled." : "Browser alerts remain off; in-app reminders stay active.");
  };
  ...
  const lifecycle = [
    { label: "New", count: tickets.filter((ticket) => ticket.status === "New").length, tone: "new" },
    { label: "In progress", count: inProgress, tone: "progress" },
    { label: "Dead", count: dead, tone: "dead" },
    { label: "Resolved", count: resolved, tone: "resolved" },
    { label: "Closed", count: closed, tone: "closed" },
    { label: "Invalid", count: invalid, tone: "invalid" },
  ];

  const renderMobileOperations = () => <div className="mobile-ops-stage"><div className="mobile-ops-shell" data-testid="mobile-ops-workspace">
    ...
    <main className="mobile-ops-content">
      {mobileTab === "Pulse" && <><section className="mobile-ops-welcome"><div><span className="live-dot" /> Live property pulse</div><h1>Good evening, hotel team.</h1><p>{reminderTickets.length} active requests are connected to the guest experience.</p></section><section className="mobile-ops-metrics" aria-label="Operations key metrics">{metrics.map((metric) => <article key={metric.label} className={metric.alert ? "alert" : ""}><span>{metric.label}</span><strong>{metric.value}</strong><small>{metric.note}</small></article>)}</section><section className="mobile-ops-lifecycle"><div className="mobile-ops-section-head"><div><span className="section-label">Request lifecycle</span><h2>Today at a glance</h2></div><span className="mobile-ops-live"><i className="live-dot" /> Live</span></div><div className="mobile-ops-lifecycle-bars">{lifecycle.map((item) => <div key={item.label}><span>{item.label}</span><div><i className={`lifecycle-${item.tone}`} style={{ width: `${Math.max(item.count ? 12 : 4, Math.round(item.count / Math.max(1, raised) * 100))}%` }} /></div><strong>{item.count}</strong></div>)}</div></section><section className="mobile-ops-section"><div className="mobile-ops-section-head"><div><span className="section-label">Needs attention</span><h2>Act next</h2></div><button onClick={() => setMobileTab("Requests")}>View all</button></div>{reminderTickets.slice(0, 4).map((ticket) => <article className="mobile-ops-attention" key={ticket.id}><span className={`ticket-status-dot ${statusClass(ticket.status)}`} /><div><strong>{ticket.title}</strong><small>{ticket.id} · Room {ticket.room} · {ticket.queueMinutes}m queue</small></div>{ticket.status === "New" ? <button onClick={() => changeStatus(ticket.id, "In progress")}>Accept</button> : ticket.status === "Dead" ? <button onClick={() => changeStatus(ticket.id, "In progress")}>Recover</button> : <button onClick={() => changeStatus(ticket.id, "Resolved")}>Resolve</button>}</article>)}</section></>}
  ...
  if (!session && mobileExperience) return <div className="mobile-ops-stage"><main className="mobile-ops-shell mobile-ops-auth" data-testid="mobile-ops-auth">
    <header><button className="mobile-ops-back" onClick={returnToGuestSignIn} aria-label="Back to guest sign-in"><ChevronLeftIcon /></button><AppMark compact /><span className="mobile-ops-mode">Hotel team</span></header>
    <section className="mobile-ops-auth-copy"><span className="eyebrow">Protected partner access</span><h1>Run the stay from the same Intellistay experience.</h1><p>Sign in with an approved hotel email or participating-property login. Guest and hotel records stay separated.</p></section>
    <section className="mobile-ops-auth-card"><div className="ops-access-tabs" role="tablist" aria-label="Hotel operations access method"><button role="tab" aria-selected={accessMode === "email"} className={accessMode === "email" ? "active" : ""} onClick={() => { setAccessMode("email"); setAuthError(""); }}>Hotel email</button><button role="tab" aria-selected={accessMode === "partner"} className={accessMode === "partner" ? "active" : ""} onClick={() => { setAccessMode("partner"); setAuthError(""); }}>Partner login</button></div>{accessMode === "email" ? <label className="ops-auth-field"><span>Work email</span><input value={staffEmail} onChange={(event) => setStaffEmail(event.target.value)} type="email" aria-label="Hotel work email" /><small>Demo: manager@auroragrand.demo</small></label> : <><label className="ops-auth-field"><span>Participating hotel</span><select value={partnerHotel} onChange={(event) => setPartnerHotel(event.target.value)}>{hotels.map((item) => <option key={item.name}>{item.name}</option>)}</select></label><label className="ops-auth-field"><span>Partner ID</span><input value={partnerId} onChange={(event) => setPartnerId(event.target.value)} /></label><label className="ops-auth-field"><span>Access code</span><input value={accessCode} onChange={(event) => setAccessCode(event.target.value)} type="password" /><small>Demo: INTELLI-DEMO</small></label></>}{authError && <p className="ops-auth-error" role="alert"><ExclamationTriangleIcon />{authError}</p>}<button className="ops-auth-submit" onClick={authenticate}>Enter hotel workspace <ChevronRightIcon /></button></section>
    <button className="mobile-ops-return" onClick={returnToGuestSignIn}><ChevronLeftIcon /> Return to reservation or guest access</button>
  </main></div>;

  if (!session) return <div className="ops-auth-surface">
    <button className="ops-auth-guest-switch" onClick={returnToGuestSignIn}><ChevronLeftIcon /><span>Guest sign-in</span></button>
  ...
  if (mobileExperience) return renderMobileOperations();
  ...
  return <div className="web-ops-surface ops-v2">
    <button className="ops-switch-guest" onClick={returnToGuestSignIn}><ChevronLeftIcon /><span><strong>Guest sign-in</strong><small>Reservation or guest access</small></span><ChevronRightIcon /></button>
    ...
    <main className="web-main ops-main"><header className="web-topbar ops-topbar"><div><span className="eyebrow">{copy.eyebrow}</span><h1>{copy.title}</h1><p>{copy.description}</p></div><div className="ops-top-actions"><button className="ops-action-secondary" onClick={emailWebReport}><EnvelopeClosedIcon /> Email report</button><button className="ops-action-primary" onClick={exportWebReport}><DownloadIcon /> Download</button><button className="ops-icon-button" aria-label="Operations notifications" aria-expanded={notificationCenterOpen} onClick={() => { setNotificationCenterOpen((open) => !open); setRemindersAcknowledged(true); }}><BellIcon />{!remindersAcknowledged && reminderTickets.length > 0 && <i />}</button></div></header>{notificationCenterOpen && <aside className="ops-notification-panel" role="dialog" aria-label="Operations notifications"><header><div><span className="eyebrow">Hourly follow-up</span><h2>Operations notifications</h2></div><button onClick={() => setNotificationCenterOpen(false)} aria-label="Close operations notifications"><Cross2Icon /></button></header><p>Every open request is reminded immediately and again every hour until its status changes.</p><div>{reminderTickets.map((ticket) => <button key={ticket.id} onClick={() => { setRequestSearch(ticket.id); setRequestFilter("All"); setActiveSection("Requests"); setNotificationCenterOpen(false); }}><span className={`ticket-status-dot ${statusClass(ticket.status)}`} /><span><strong>{ticket.title}</strong><small>{ticket.id} · {ticket.status} · Room {ticket.room} · next hourly follow-up scheduled</small></span><ChevronRightIcon /></button>)}</div>{!reminderTickets.length && <div className="ops-empty"><CheckCircledIcon /><strong>No active reminders.</strong><span>All guest requests have completed their action loop.</span></div>}<button className="ops-action-primary full" onClick={() => void enableBrowserAlerts()}><BellIcon /> {browserAlertsEnabled ? "Browser alerts enabled" : "Enable browser alerts"}</button></aside>}
