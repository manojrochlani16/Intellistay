...
export function WebOperationsDashboard() {
  ...
  useEffect(() => {
    ...
    const fit = () => setMobileOpsScale(Math.min(1, Math.max(.56, Math.min((window.innerWidth - 24) / 393, (window.innerHeight - 24) / 852))));
    fit();
  ...
  if (!session && mobileExperience) return <div className="mobile-ops-stage"><div className="mobile-ops-scale-box" style={mobileOpsFrameStyle}><main className="mobile-ops-shell mobile-ops-auth" style={mobileOpsShellStyle} data-testid="mobile-ops-auth">
    <header className="mobile-ops-auth-header"><button className="mobile-ops-back" onClick={returnToGuestSignIn} aria-label="Back to guest sign-in"><ChevronLeftIcon /></button><div className="mobile-ops-auth-brand" aria-label="Intellistay hotel operations"><span>Intellistay</span><strong>Hotel operations</strong></div><span className="mobile-ops-auth-secure" aria-hidden="true"><IdCardIcon /></span></header>
    <section className="mobile-ops-auth-copy"><span className="eyebrow">Protected staff workspace</span><h1>One clear view of every guest request.</h1><p>Sign in with an approved hotel email or participating-property account.</p></section>
    ...
    <div className="mobile-ops-auth-note"><CheckCircledIcon /><span><strong>Property-scoped access</strong><small>Guest and hotel records remain separated.</small></span></div><button className="mobile-ops-return" onClick={returnToGuestSignIn}>Return to guest access</button>
  </main></div></div>;
